...
START TRANSACTION;
SELECT quantidade_estoque FROM alimentos WHERE id = 1 FOR UPDATE;
-- valida se tem estoque suficiente
UPDATE alimentos SET quantidade_estoque = quantidade_estoque - 1 WHERE id = 1;
UPDATE retiradas SET status = 'confirmada', confirmado_em = NOW() WHERE id = 1;
COMMIT;
SHOW TABLE STATUS LIKE 'alimentos'; -- REVISAO NECESSARIA, FEITO PELA IA E NN VERIFIQUEI AINDA
